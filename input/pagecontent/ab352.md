...
flowchart TD
    ...
    G1 -- Yes --> H1[Deny\nObligation: log-denial]
    G1 -- No --> I1[Continue evaluation\n(local org policies)]
    F1 -- No --> I1

    E1 -- No --> E2{Action = access/use?}
    E2 -- Yes --> F2{Recipient jurisdiction = CA\nAND purpose = TREATMENT?}
    F2 -- Yes --> H2[Permit\nObligation: segment-sensitive-data]
    F2 -- No --> I2[NotApplicable or other org policies]

    E2 -- No --> I2

    H2 --> J[PEP enforces segmentation\n(EHR/HIE data segregation)]
    H1 --> K[PEP denies disclosure\nand records AuditEvent]
